...
PY
python scripts/build_chunk_index.py --jsonl /N/slate/ds85/bgcmodel_data/mega_all.jsonl \
    --max-seq-len 262144 --chunk-overlap 2048
```

## 6. THE ENGINEERING: multi-GPU long context (do the 2-GPU prototype FIRST)

Our harness (`scripts/finetune_evo2_lora.py`) is **single-GPU DeepSpeed**. To fit L≥131k the
