...
**DO NOT read on startup.** `grep` it by symptom when something breaks.

```bash
grep -ni "OOM\|CUDA" docs/bugs.md
grep -n "^## " docs/bugs.md          # list subject sections
```

**Why this is separate from `memory.md`.** Different retrieval key. Results are looked up by
**date/phase**; bugs are looked up by **symptom**. Folding these into a chronological ledger would
scatter one symptom across years of unrelated context. This file is indexed by **subject**, and
that is deliberate.

**Entry format.** Under the right subject heading:

```
**[Symptom]** what you actually observe — the error text, the wrong number, the silence.
**[Cause]** the real mechanism.
**[Proven fix]** what actually worked. Not what should work.
**[Severity]** note it here if it corrupted a result.
```

⚠️ **The highest-severity bug class in this repo is SILENT DEGRADATION** — a resource missing, a
gate defaulting to its passing value, a config argument that is accepted and then ignored. These
produce numbers, not errors. `BGC_EVAL_STRICT` exists because of them. When you fix one, record
**what the wrong number looked like**, because that is what the next person will recognise.
