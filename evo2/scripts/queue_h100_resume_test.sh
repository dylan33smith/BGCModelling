...
usage() {
  cat <<'EOF'
Usage:
  evo2/scripts/queue_h100_resume_test.sh [options]
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  log "  deepspeed --num_gpus=1 evo2/scripts/finetune_evo2_lora.py ${COMMON_ARGS[*]} --output-dir $PHASE1_DIR --max-steps 5 --save-every 2"
else
  set +e
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  log "  deepspeed --num_gpus=1 evo2/scripts/finetune_evo2_lora.py ${COMMON_ARGS[*]} --output-dir $PHASE2_DIR --max-steps 5 --save-every 99 --resume-from $RESUME_CKPT"
else
  set +e
  HF_HOME="$HF_HOME_PATH" \
    micromamba run -n "$ENV_NAME" \
