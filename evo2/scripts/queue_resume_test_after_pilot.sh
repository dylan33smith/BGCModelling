...
#   nohup micromamba run -n bgcmodel bash evo2/scripts/queue_resume_test_after_pilot.sh \
...
usage() {
  cat <<'EOF'
Usage:
  evo2/scripts/queue_resume_test_after_pilot.sh --pilot-dir RUN_DIR [options] [-- RESUME_TEST_ARGS...]
...
evo2/scripts/queue_h100_resume_test.sh (which does its own GPU-idle gating).
